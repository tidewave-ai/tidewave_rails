...
describe Tidewave::DatabaseAdapters::Sequel do
  ...
  describe "adapter and database information" do
    ...
    context "with PostgreSQL database" do
      it "includes correct adapter and database names in execute_query response" do
        db = double("Sequel::Database")
        result_set = double("Result")
        ...
        allow(db).to receive(:fetch).and_return(result_set)
        allow(result_set).to receive(:all).and_return([ { test_column: 1 } ])

        query = "SELECT 1 as test_column"
        response = adapter.execute_query(query)

        expect(response[:adapter]).to eq("POSTGRESQL")
        expect(response[:database]).to eq("postgres_db")
      end
    end
    ...
    context "with MySQL database" do
      it "includes correct adapter and database names in execute_query response" do
        ...
        expect(response[:adapter]).to eq("MYSQL2")
        expect(response[:database]).to eq("mysql_db")
      end
    end

    context "with unknown database" do
      it "handles unknown database configurations" do
        db = double("Sequel::Database")
        result_set = double("Result")
        allow(::Sequel::Model).to receive(:db).and_return(db)
        ...
        expect(response[:adapter]).to eq("UNKNOWN")
        expect(response[:database]).to be_nil
      end
    end
  end
end
